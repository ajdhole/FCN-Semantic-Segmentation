...
def load_vgg(sess, vgg_path):
    ...
    C1 = graph.get_tensor_by_name(vgg_input_tensor_name)

    vgg_keep_prob_tensor_name = 'keep_prob:0'
    keep = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)

    vgg_layer3_out_tensor_name = 'layer3_out:0'
    C3 = graph.get_tensor_by_name(vgg_layer3_out_tensor_name)

    vgg_layer4_out_tensor_name = 'layer4_out:0'
    C4 = graph.get_tensor_by_name(vgg_layer4_out_tensor_name)

    vgg_layer7_out_tensor_name = 'layer7_out:0'
    C7 = graph.get_tensor_by_name(vgg_layer7_out_tensor_name)

    return C1, keep, C3, C4, C7
...
def run():
    ...
    with tf.Session(config=config) as sess:
        ...
        image_input, keep_prob, layer_3, layer_4, layer_7 = load_vgg(sess, vgg_path)
